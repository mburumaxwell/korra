...
#define WIFI_MGMT_EVENTS                                                                                               \
  (NET_EVENT_WIFI_CONNECT_RESULT | NET_EVENT_WIFI_DISCONNECT_RESULT | NET_EVENT_WIFI_SIGNAL_CHANGE |                   \
...
static int process_certificates(struct wifi_cert_data *certs, size_t count) {
  for (size_t i = 0; i < count; i++) {
    ...
    LOG_DBG("Found credential type: %-18s -> tag: %-23s (len: %d)", tls_credential_type_txt(certs[i].type),
            korra_credential_tag_type_txt(certs[i].tag), len);
...
static void wifi_scan_event_handler(struct net_mgmt_event_callback *cb, uint64_t mgmt_event, struct net_if *iface) {
  if (mgmt_event == NET_EVENT_WIFI_SCAN_RESULT) {
    ...
    if (scan_result == 1U) {
      printk("%-4s | %-32s | %-13s | %-4s | %-20s | %-17s | %-8s\n", "Num", "SSID", "Chan (Band)", "RSSI", "Security",
             "BSSID", "MFP");
    }
    ...
    if (entry->mac_length) snprintf(mac_string_buf, sizeof(mac_string_buf), FMT_LL_ADDR_6, PRINT_LL_ADDR_6(entry->mac));

    printk(
        "%-4d | %-32s | %-4u (%-6s) | %-4d | %-20s | %-17s | %-8s\n", scan_result, ssid_print, entry->channel,
        wifi_band_txt(entry->band), entry->rssi,
        ((entry->wpa3_ent_type) ? wifi_wpa3_enterprise_txt(entry->wpa3_ent_type) : wifi_security_txt(entry->security)),
        ((entry->mac_length) ? mac_string_buf : (const uint8_t *)""), wifi_mfp_txt(entry->mfp));
