...
//
// Time sync requires internet so we rely on L4 callbacks from connectivity manager which works irrespective of the
// network in use. Should internet fail, the timer is stopped then restarted when internet resumes.
