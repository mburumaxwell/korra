  ...
  for (size_t i = 0; i < ARRAY_SIZE(credentials); i++) {
    ...
    if (ret) {
      LOG_WRN("Unable to determine if type: %-18s -> tag: %-20s exists (ret: %d)",
              tls_credential_type_txt(credentials[i].type), korra_credential_tag_type_txt(credentials[i].tag), ret);
      continue;
    }
    ...
    LOG_DBG("Deleting type: %-18s -> tag: %-20s", tls_credential_type_txt(credentials[i].type),
            korra_credential_tag_type_txt(credentials[i].tag));

    ret = tls_credential_delete(credentials[i].tag, credentials[i].type);
    if (ret) {
      LOG_WRN("Unable to delete type: %-18s -> tag: %-20s (ret: %d)", tls_credential_type_txt(credentials[i].type),
              korra_credential_tag_type_txt(credentials[i].tag), ret);
      continue;
    }
  }
...
int tls_credential_exists(enum korra_credential_tag_type tag, enum tls_credential_type type, size_t known_len,
                          bool *exists) {
  size_t len;
...
static int generate_cert(enum korra_credential_tag_type tag, const char *devid, const size_t devid_len,
                         const char *not_before, const char *not_after, uint8_t *cert_pem, size_t cert_pem_len,
  ...
  mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &entropy, (const unsigned char *)seed, strlen(seed));

  // Generate EC key pair (secp256r1)
  mbedtls_pk_setup(&key, mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY));
  mbedtls_ecp_gen_key(MBEDTLS_ECP_DP_SECP256R1, mbedtls_pk_ec(key), mbedtls_ctr_drbg_random, &ctr_drbg);
  ...
  ret = mbedtls_x509write_crt_pem(&crt, cert_pem, cert_pem_len, mbedtls_ctr_drbg_random, &ctr_drbg);

  if (ret) {
    LOG_ERR("Unable to generate certificate for tag: %-20s: %d (mbedtls: -0x%04X)", korra_credential_tag_type_txt(tag),
            ret, -ret);
  }
...
static int generate_credentials(const char *devid, const size_t devid_len) {
  ...
  struct tm tm;
  gmtime_r(&now, &tm);
  char not_before[sizeof(
      "20241231235959")]; // format: YYYYMMDDhhmmss e.g. 20241231235959 for December 31st 2024 at 23:59:59
  ...
  for (size_t i = 0; i < ARRAY_SIZE(generateble_credentials); i++) {
    ...
    if (ret) {
      LOG_WRN("Unable to determine if tag: %-20s exists (ret: %d)",
              korra_credential_tag_type_txt(generateble_credentials[i].tag), ret);
      continue;
    }

    if (exists) {
      LOG_DBG("Credential for tag: %-20s already exists",
    ...
    if (generate) {
      ret = generate_cert(generateble_credentials[i].tag, devid, devid_len, not_before, not_after, cert_pem,
...
static int load_credentials() {
  ...
  for (size_t i = 0; i < ARRAY_SIZE(credentials); i++) {
    ...
    if (exists) {
      LOG_DBG("Credential for type: %-18s -> tag: %-20s already exists", tls_credential_type_txt(credentials[i].type),
              korra_credential_tag_type_txt(credentials[i].tag));
      continue;
    }

    LOG_DBG("Adding type: %-18s -> tag: %-20s (len: %d)", tls_credential_type_txt(credentials[i].type),
            korra_credential_tag_type_txt(credentials[i].tag), credentials[i].len);

    if (credentials[i].data == NULL || credentials[i].len <= 0) {
      LOG_ERR("No data/len for type: %-18s -> tag: %-20s (len: %d)", tls_credential_type_txt(credentials[i].type),
              korra_credential_tag_type_txt(credentials[i].tag), credentials[i].len);
      return -EFAULT;
    }
    ...
    if (ret < 0) {
      LOG_ERR("Failed to add type: %-18s -> tag: %-20s (ret: %d)", tls_credential_type_txt(credentials[i].type),
              korra_credential_tag_type_txt(credentials[i].tag), ret);
      return ret;
    }
  }
