...
// extern definition of this in SimpleSerialShell.h, so all users of the
// class will have visibility to this reference.
...
public:
  Command(const __FlashStringHelper *n, CommandFunction f) : nameAndDocs(n), myFunc(f) {};

  int execute(int argc, char **argv) { return myFunc(argc, argv); };
...
static char *quoteAwareTokenizer(char *str, const char *delim, char **saveptr) {
  char *s;
  // first call? use str; otherwise continue from *saveptr
  if (str) {
    s = str;
  } else {
    s = *saveptr;
  ...
  char *start;
  // if starts with a quote, consume until next quote
  if (*s == '"') {
    start = s + 1;
    s = start;
    while (*s && *s != '"') {
    ...
    // no quote: consume until next delimiter
    start = s;
...
SimpleSerialShell::SimpleSerialShell()
    : shellConnection(NULL), m_lastErrNo(EXIT_SUCCESS),
      //   tokenizer(strtok_r)
      tokenizer(quoteAwareTokenizer) {
  resetBuffer();
